# props_predict_threes.py
# Generates NBA player 3-pointers made over/under predictions for today's props lines.
# P(over) = Normal CDF((pred_threes - line) / sigma)
#
# Usage: python props_predict_threes.py
...
PROPS_MIN_EDGE     = 0.12
STAR_3P_CUTOFF     = 2.5    # avg 3PM above which suppress unders (shooters)
STAR_LINE_BUFFER   = 1.0    # skip unders when line is within 1 of season avg
PROPS_MARKET       = "player_threes"
...
def load_todays_threes_props(conn) -> pd.DataFrame:
...
def build_player_features(props_df, conn, feature_cols) -> pd.DataFrame:
    logs = pd.read_sql("""
        SELECT player_id, player_name, team_abbreviation, game_id, game_date,
               season, matchup, is_home, min_played, fg3m
        FROM player_game_logs
        WHERE fg3m IS NOT NULL
    ...
    for _, prop in props_df.iterrows():
        ...
        threes_l3  = hist["fg3m"].tail(3).mean()
        threes_l5  = hist["fg3m"].tail(5).mean()
        threes_l10 = hist["fg3m"].tail(10).mean()
        threes_std = hist["fg3m"].tail(10).std()
        min_l5     = hist["min_played"].tail(5).mean()
        season_threes = hist[hist["season"] == season]["fg3m"].mean()

        home_3s = hist[hist["is_home"]==1]["fg3m"].tail(5).mean()
        away_3s = hist[hist["is_home"]==0]["fg3m"].tail(5).mean()
        ...
        feat = {
            ...
            "season_threes":  season_threes if not np.isnan(season_threes) else threes_l10,
            "threes_home_l5": home_3s if not np.isnan(home_3s) else threes_l5,
            "threes_away_l5": away_3s if not np.isnan(away_3s) else threes_l5,
        }
...
def generate_predictions(features_df, model, feature_cols, sigma) -> pd.DataFrame:
    ...
    pred_threes = model.predict(X)

    df = features_df.copy()
    df["pred_threes"] = pred_threes

    df["over_prob"]  = df.apply(
        lambda r: float(norm.cdf((r["pred_threes"] - r["line"]) / sigma))
    ...
    # Shooter guard: don't fade a volume 3-point shooter near their season avg
    if "season_threes" in df.columns:
        star_mask = (
            (df["season_threes"] > STAR_3P_CUTOFF) &
            (df["line"] >= df["season_threes"] - STAR_LINE_BUFFER)
        )
        suppressed = star_mask & (df["under_value"] == 1)
        if suppressed.any():
            for name in df.loc[suppressed, "player_name"]:
                print(f"  [guard] Suppressed 3P under for {name} — line near season avg")
...
def save_predictions(df, conn):
    ...
    conn.execute("DELETE FROM props_threes_predictions WHERE predict_date=?", (today,))
    df["predict_date"]  = today
    df["actual_threes"] = None

    save_cols = [c for c in df.columns if c in [
        "player_name","game_id","predict_date","home_team","away_team",
        "commence_time","bookmaker","line","pred_threes",
        ...
        "props_sigma","actual_threes",
    ]]
    df[save_cols].to_sql("props_threes_predictions", conn,
                         if_exists="append", index=False, chunksize=100)
    conn.commit()
    print(f"  3P predictions saved -> nba.db: props_threes_predictions")


if __name__ == "__main__":
    print("\n-- Props Predict: Player 3-Pointers --------------------------------")

    for fname in ["props_threes_model.pkl", "props_threes_features.json",
                  "props_threes_model_std.json"]:
        if not Path(fname).exists():
            print(f"  ERROR: {fname} not found. Run python train_props_threes.py first.")
            exit(1)

    with open("props_threes_model.pkl", "rb") as f:
        model = pickle.load(f)
    with open("props_threes_features.json") as f:
        feature_cols = json.load(f)
    with open("props_threes_model_std.json") as f:
        sigma = json.load(f)["rmse"]

    print(f"  Model loaded. sigma={sigma:.2f} 3pm. Features: {len(feature_cols)}")

    conn = get_conn()

    props_df = load_todays_threes_props(conn)
    if props_df.empty:
        print("  No 3-pointer lines found. Run python props_odds.py first.")
        conn.close()
        exit()
    print(f"  Found {len(props_df)} player 3P line(s) for today.")
    ...
    print(f"\n{'='*60}")
    print(f"  PLAYER 3-POINTERS PICKS  (min edge: {PROPS_MIN_EDGE:.0%})")
    print(f"{'='*60}")
    ...
    for _, g in results.sort_values("over_edge", ascending=False).head(15).iterrows():
        ...
        print(f"    Pred: {g['pred_threes']:.1f}  Line: {line}  "
              f"P(over): {g['over_prob']:.1%}  Over edge: {g['over_edge']:+.1%}")
        if g["over_value"]:
            print(f"    >> VALUE: OVER {line} 3PM")
        elif g["under_value"]:
            print(f"    >> VALUE: UNDER {line} 3PM")
        print()
