# train_props_threes.py
# Trains an XGBoost regression model to predict NBA player 3-pointers made.
# Target column = fg3m (three-pointers made).
#
# Usage: python train_props_threes.py
# Output: props_threes_model.pkl, props_threes_features.json, props_threes_model_std.json
...
def build_features(conn) -> pd.DataFrame:
    logs = pd.read_sql("""
        SELECT player_id, player_name, team_abbreviation, game_id,
               game_date, season, matchup, is_home, wl, min_played, fg3m
    ...
    for player_id, grp in logs.groupby("player_id"):
        ...
        for i in range(MIN_GAMES, len(grp)):
            ...
            season_threes = hist[hist["season"] == cur["season"]]["fg3m"].mean()

            home_hist = hist[hist["is_home"] == 1]["fg3m"]
            away_hist = hist[hist["is_home"] == 0]["fg3m"]
            threes_home_l5 = home_hist.tail(5).mean() if len(home_hist) >= 3 else threes_l5
            threes_away_l5 = away_hist.tail(5).mean() if len(away_hist) >= 3 else threes_l5
            ...
            rows.append({
                ...
                "threes_l3":      threes_l3,
                "threes_l5":      threes_l5,
                "threes_l10":     threes_l10,
                "threes_std":     threes_std if not np.isnan(threes_std) else 0.0,
                "min_l5":         min_l5,
                "season_threes":  season_threes if not np.isnan(season_threes) else threes_l10,
                "threes_home_l5": threes_home_l5,
                "threes_away_l5": threes_away_l5,
                "is_home":        int(cur["is_home"]),
                "days_rest":      days_rest,
                "opp_def_rtg":    opp_def_rtg,
                "opp_pace":       opp_pace,
                "opp_opp_pts":    opp_opp_pts,
                "target_threes":  cur["fg3m"],
            })
...
FEATURE_COLS = [
    "threes_l3", "threes_l5", "threes_l10", "threes_std",
    "min_l5", "season_threes",
    "threes_home_l5", "threes_away_l5",
    "is_home", "days_rest",
...
if __name__ == "__main__":
    print("\n-- Train Props: Player 3-Pointers Model ----------------------------")
    ...
    df = df.dropna(subset=FEATURE_COLS + ["target_threes"])
    print(f"  Training rows: {len(df):,} | Players: {df['player_id'].nunique()}")

    X = df[FEATURE_COLS].values
    y = df["target_threes"].values
    ...
    print(f"  CV RMSE: {cv_rmse:.2f} 3pm")

    model.fit(X, y)
    train_preds = model.predict(X)
    train_rmse  = math.sqrt(mean_squared_error(y, train_preds))
    print(f"  Train RMSE: {train_rmse:.2f} 3pm")
    ...
    with open("props_threes_model.pkl", "wb") as f:
        pickle.dump(model, f)
    with open("props_threes_features.json", "w") as f:
        json.dump(FEATURE_COLS, f)
    with open("props_threes_model_std.json", "w") as f:
        json.dump({"rmse": train_rmse, "cv_rmse": cv_rmse}, f)

    print(f"\n  Saved: props_threes_model.pkl | sigma={train_rmse:.2f} 3pm")
