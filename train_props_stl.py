# train_props_stl.py
# Trains an XGBoost regression model to predict NBA player steals.
# Target column = stl (steals per game).
#
# Usage: python train_props_stl.py
# Output: props_stl_model.pkl, props_stl_features.json, props_stl_model_std.json
...
def build_features(conn) -> pd.DataFrame:
    logs = pd.read_sql("""
        SELECT player_id, player_name, team_abbreviation, game_id,
               game_date, season, matchup, is_home, wl, min_played, stl
    ...
    for player_id, grp in logs.groupby("player_id"):
        ...
        for i in range(MIN_GAMES, len(grp)):
            ...
            season_stl = hist[hist["season"] == cur["season"]]["stl"].mean()

            home_hist = hist[hist["is_home"] == 1]["stl"]
            away_hist = hist[hist["is_home"] == 0]["stl"]
            stl_home_l5 = home_hist.tail(5).mean() if len(home_hist) >= 3 else stl_l5
            stl_away_l5 = away_hist.tail(5).mean() if len(away_hist) >= 3 else stl_l5
            ...
            rows.append({
                ...
                "stl_l5":      stl_l5,
                "stl_l10":     stl_l10,
                "stl_std":     stl_std if not np.isnan(stl_std) else 0.0,
                "min_l5":      min_l5,
                "season_stl":  season_stl if not np.isnan(season_stl) else stl_l10,
                "stl_home_l5": stl_home_l5,
                "stl_away_l5": stl_away_l5,
...
FEATURE_COLS = [
    "stl_l3", "stl_l5", "stl_l10", "stl_std",
    "min_l5", "season_stl",
    "stl_home_l5", "stl_away_l5",
    "is_home", "days_rest",
...
if __name__ == "__main__":
    print("\n-- Train Props: Player Steals Model --------------------------------")
    ...
    df = df.dropna(subset=FEATURE_COLS + ["target_stl"])
    print(f"  Training rows: {len(df):,} | Players: {df['player_id'].nunique()}")

    X = df[FEATURE_COLS].values
    y = df["target_stl"].values
    ...
    print(f"  CV RMSE: {cv_rmse:.2f} stl")

    model.fit(X, y)
    train_preds = model.predict(X)
    train_rmse  = math.sqrt(mean_squared_error(y, train_preds))
    print(f"  Train RMSE: {train_rmse:.2f} stl")
    ...
    with open("props_stl_model.pkl", "wb") as f:
        pickle.dump(model, f)
    with open("props_stl_features.json", "w") as f:
        json.dump(FEATURE_COLS, f)
    with open("props_stl_model_std.json", "w") as f:
        json.dump({"rmse": train_rmse, "cv_rmse": cv_rmse}, f)

    print(f"\n  Saved: props_stl_model.pkl | sigma={train_rmse:.2f} stl")
