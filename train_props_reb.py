# train_props_reb.py
# Trains an XGBoost regression model to predict NBA player rebounds per game.
# Mirror of train_props.py — same architecture, different target column.
#
# Usage: python train_props_reb.py
# Output: props_reb_model.pkl, props_reb_features.json, props_reb_model_std.json
...
MIN_MINUTES = 10.0    # skip garbage time
...
def build_features(conn) -> pd.DataFrame:
    logs = pd.read_sql("""
        SELECT player_id, player_name, team_abbreviation, game_id,
               game_date, season, matchup, is_home, wl, min_played, reb
    ...
    # Opponent defensive stats from team_season_stats
    ...
    for player_id, grp in logs.groupby("player_id"):
        ...
        for i in range(MIN_GAMES, len(grp)):
            ...
            season_reb = hist[hist["season"] == cur["season"]]["reb"].mean()

            home_hist = hist[hist["is_home"] == 1]["reb"]
            away_hist = hist[hist["is_home"] == 0]["reb"]
            reb_home_l5 = home_hist.tail(5).mean() if len(home_hist) >= 3 else reb_l5
            reb_away_l5 = away_hist.tail(5).mean() if len(away_hist) >= 3 else reb_l5
            ...
            rows.append({
                ...
                "reb_l5":      reb_l5,
                "reb_l10":     reb_l10,
                "reb_std":     reb_std if not np.isnan(reb_std) else 0.0,
                "min_l5":      min_l5,
                "season_reb":  season_reb if not np.isnan(season_reb) else reb_l10,
                "reb_home_l5": reb_home_l5,
                "reb_away_l5": reb_away_l5,
...
FEATURE_COLS = [
    "reb_l3", "reb_l5", "reb_l10", "reb_std",
    "min_l5", "season_reb",
    "reb_home_l5", "reb_away_l5",
    "is_home", "days_rest",
...
if __name__ == "__main__":
    print("\n-- Train Props: Player Rebounds Model ------------------------------")
    ...
    df = df.dropna(subset=FEATURE_COLS + ["target_reb"])
    print(f"  Training rows: {len(df):,} | Players: {df['player_id'].nunique()}")

    X = df[FEATURE_COLS].values
    y = df["target_reb"].values
    ...
    print(f"  CV RMSE: {cv_rmse:.2f} reb")

    model.fit(X, y)
    train_preds = model.predict(X)
    train_rmse  = math.sqrt(mean_squared_error(y, train_preds))
    print(f"  Train RMSE: {train_rmse:.2f} reb")
    ...
    with open("props_reb_model.pkl", "wb") as f:
        pickle.dump(model, f)
    with open("props_reb_features.json", "w") as f:
        json.dump(FEATURE_COLS, f)
    with open("props_reb_model_std.json", "w") as f:
        json.dump({"rmse": train_rmse, "cv_rmse": cv_rmse}, f)

    print(f"\n  Saved: props_reb_model.pkl | sigma={train_rmse:.2f} reb")
